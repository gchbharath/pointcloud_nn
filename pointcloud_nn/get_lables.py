...
def lables_ros(confidences, strategy = "bayes"):
    dt = 0.1
    dict_confidences = {}
    prior = 0.5
    semantic_config = yaml.safe_load(open("/home/bharath/PEAMS/robust_SLAM/ROS2_ws/src/pointcloud_nn/config/semantic-kitti-mos.yaml"))
    #confidences = confidences[0]
    #print("label ROS",(confidences[0].shape))
    ...
    try:
        if strategy == "bayes":
            confidence = np.array(confidences[0][0][-1])
            log_odds = prob_to_log_odds(confidence)
            for i in range(4):
                confidence = np.array(confidences[i+1][0][-2-i])
                log_odds += prob_to_log_odds(confidence)
                log_odds -= prob_to_log_odds(prior * np.ones_like(confidence))
            final_confidence = log_odds_to_prob(log_odds)
            pred_labels = to_label(final_confidence, semantic_config)
        #verify_predictions(3, pred_path, semantic_config)
    except:
        return np.array([99])
    return pred_labels
